...
let board_name = '제 1 게시판';
let board_address = 'board_1';

// 글 목록 페이지
router.get('/list/:page', function(req, res, next) {
  MySqlHandler.board_1.query(`SELECT * FROM \`board\` ORDER BY \`no\` DESC LIMIT ${15 * (req.params.page - 1)},15`,
        (err, rows1) => {
          if(err) {throw err}
          else {
            rows1.map(x => board_func.dateform_list_time(x));
            MySqlHandler.board_1.query('SELECT COUNT(*) FROM \`board\`',
              (err, rows2) => {
...
router.get('/search/:page', function(req, res) {
  if(req.query.search_type == 'title&contents'){
    MySqlHandler.board_1.query(`SELECT * FROM \`board\` WHERE \`title\` LIKE '%${req.query.keyword}%' OR \`contents_plain\` LIKE '%${req.query.keyword}%'
    ORDER BY \`no\` DESC`,
    (err, rows1) => {
      rows1.map(x => board_func.dateform_list_time(x));
      if(err) {throw err}
    ...
    MySqlHandler.board_1.query(`SELECT * FROM \`board\` WHERE \`title\` LIKE '%${req.query.keyword}%' ORDER BY \`no\` DESC`,
    (err, rows1) => {
      if(err) {throw err}
      else {
        res.render('list_search', {pageinfo: board_name+' 검색', greenalert: req.cookies.greenalert, yellowalert: req.cookies.yellowalert, board_name: board_name,
    ...
    MySqlHandler.board_1.query(`SELECT * FROM \`board\` WHERE \`contents_plain\` LIKE '%${req.query.keyword}%' ORDER BY \`no\` DESC`,
    (err, rows1) => {
      if(err) {throw err}
      else {
        res.render('list_search', {pageinfo: board_name+' 검색', greenalert: req.cookies.greenalert, yellowalert: req.cookies.yellowalert, board_name: board_name,
    ...
    MySqlHandler.board_1.query(`SELECT * FROM \`board\` WHERE \`writer\` LIKE '%${req.query.keyword}%' ORDER BY \`no\` DESC`,
    (err, rows1) => {
      if(err) {throw err}
      else {
        res.render('list_search', {pageinfo: board_name+' 검색', greenalert: req.cookies.greenalert, yellowalert: req.cookies.yellowalert, board_name: board_name,
...
router.post('/write', function(req, res) {
  ...
  MySqlHandler.board_1.query(`INSERT INTO \`board\` (title, contents, contents_plain, writer, writer_id, time, readnumber, commentnumber) VALUES ('${req.body.title}',
...
router.get('/read/:no', function(req, res, next) {
  // 글 내용 DB에서 데이터 가져오기
  MySqlHandler.board_1.query(`SELECT * FROM \`board\` WHERE \`no\`='${req.params.no}'`, 
        (err, rows1) => {
          if(rows1[0] == undefined) {
            ...
            // 댓글 가져오기
            MySqlHandler.board_1.query(`SELECT * FROM \`comments\` WHERE \`no\`='${req.params.no}' ORDER BY \`target\` ASC LIMIT 0,10`, 
            (err, rows2) => {
              rows2.map(x => board_func.dateform_time_comment(x));
              // 조회수 증가
              MySqlHandler.board_1.query(`UPDATE \`board\` SET readnumber = readnumber + 1 where \`no\` = '${req.params.no}'`,
              (err, rows3) => {
...
router.post('/read/:no', function(req, res, next) {
  ...
  MySqlHandler.board_1.query(`SELECT * FROM \`board\` WHERE \`no\`='${req.params.no}'`, 
    ...
    rows1.map(x => board_func.dateform_time_comment(x));
    // 댓글 가져오기
    MySqlHandler.board_1.query(`SELECT * FROM \`comments\` WHERE \`no\`='${req.params.no}' ORDER BY \`target\` ASC LIMIT ${10 * (req.body.comment_page - 1)},10`, 
    (err, rows2) => {
      rows2.map(x => board_func.dateform_time_comment(x));
      // 조회수 증가
      MySqlHandler.board_1.query(`UPDATE \`board\` SET readnumber = readnumber + 1 where \`no\` = '${req.params.no}'`,
...
router.post('/update/:no', function(req, res, next) {
  let date = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '');
  MySqlHandler.board_1.query(`UPDATE \`board\` SET \`title\` = '${req.body.title}', \`contents\` = '${req.body.contents}',
...
router.post('/delete', function(req, res, next) {
  if(req.session.loginid){
    MySqlHandler.board_1.query(`SELECT \`no\` FROM \`board\` WHERE \`no\`= ${req.body.no} AND \`writer_id\` = '${req.session.loginid.id}'`, 
    (err, rows1) => {
      if(err) {throw err}
        ...
        // 글 삭제 시퀀스
        MySqlHandler.board_1.query(`DELETE FROM \`board\` WHERE \`no\`= ${rows1[0].no}`, 
          (err, rows2) => {
            // 글에 딸린 댓글 삭제 시퀀스
            MySqlHandler.board_1.query(`DELETE FROM \`comments\` WHERE \`no\`= ${rows1[0].no}`, 
            (err, rows2) => {
              if(err) {throw err}
              else {
...
router.post('/comment', function(req, res, next) {
  let date = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '');
  if(req.session.loginid){
    MySqlHandler.board_1.query(`INSERT INTO \`comments\` (no, target, writer, writer_id, time, contents, class) VALUES ('${req.body.no}',
    0, '${req.session.loginid.name}', '${req.session.loginid.id}', '${date}', '${req.body.comment}', 0);`,
    (err, rows1) => {
      if(err) {throw err}
      else {
        MySqlHandler.board_1.query(`UPDATE \`board\` SET commentnumber = commentnumber + 1 where \`no\` = '${req.body.no}'`,
          (err, rows2) => {
            MySqlHandler.board_1.query(`UPDATE \`comments\` SET target = LAST_INSERT_ID() where \`id\` = LAST_INSERT_ID()`,
              (err, rows3) => {
...
router.post('/deletecomment', function(req, res, next) {
  if(req.session.loginid){
    if(req.session.loginid.id == req.body.writer_id){
      MySqlHandler.board_1.query(`SELECT EXISTS (SELECT * FROM \`comments\` WHERE \`class\`= 1 and \`target\` = '${req.body.commentid}') as success;`, 
        (err, rows1) => {
          if(err) {throw err}
            ...
            MySqlHandler.board_1.query(`DELETE FROM \`comments\` WHERE \`id\`= ${req.body.commentid}`,
            (err, rows2) => {
              if(err) {throw err}
              else {
                MySqlHandler.board_1.query(`UPDATE \`board\` SET commentnumber = commentnumber - 1 where \`no\` = '${req.body.no}'`,
...
router.post('/delete_c_comment', function(req, res, next) {
  if(req.session.loginid){
    if(req.session.loginid.id == req.body.c_comment_writer_id){
      MySqlHandler.board_1.query(`DELETE FROM \`comments\` WHERE \`id\`= ${req.body.c_comment_id}`,
      (err, rows1) => {
        if(err) {throw err}
        else {
          MySqlHandler.board_1.query(`UPDATE \`board\` SET commentnumber = commentnumber - 1 where \`no\` = '${req.body.no}'`,
